...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 3) Check if user still exists: if user account is deleted when token is valid
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies && req.cookies.jwt) {
    try {
      ...
      // 2) Check if user still exists: if user account is deleted when token is valid
      const currentUser = await User.findById(decoded.id);
      if (!currentUser) {
        return next();
      }
...
// Wrapper function that returns a middleware function bc cannot pass arguments into middleware function
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  await user.save(); // Use save instead of update to run validators
